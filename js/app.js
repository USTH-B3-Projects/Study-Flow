...
function initShell() {
    ...
    const name = u.studentName || u.studentId;
...
function initDashboard() {
    ...
    const name = u.studentName || u.studentId,
        ...
        // Task services are still being implemented; keep course management usable.
    ...
    const completed = tasks.filter(
        ...
        overdue = (taskService.getOverdueTasks(u.studentId) ?? []).length;
    $("#stats").innerHTML =
...
function initCourse() {
    ...
    let filter = "all",
        sort = "priority";
    const render = () => {
        const raw = taskService.getTasksByCourseId(courseId) ?? [],
        ...
        $("#courseRecommend").onclick = () => {
            const r = (smartService.getLocalRecommendations(courseId) ?? []).slice(0, 3);
            modal(
